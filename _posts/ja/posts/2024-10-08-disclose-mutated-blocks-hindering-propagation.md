...
Aixcoin Core v25.0より前のバージョンでは、変異ブロックを送信するピアが、
...
Aixcoin Coreは、たとえばヘッダーのマークルルートや、
コインベーストランザクション内のwitness commitmentがブロック内のトランザクションと一致しない場合、
そのブロックを変異ブロックとして扱います。

Aixcoin Core v25.0より前のバージョンでは、ピアが要求されていない変異ブロックを送信することで、
...
Aixcoin Coreはコンパクトブロックの再構築状態を忘れてしまいます。変異ブロックの後に届いた`blocktxn`応答は、
ブロックの再構築には使用できませんでした。これはブロックの伝播を妨げました。

これは[#27608](https://github.com/aixcoin/aixcoin/pull/27608)で、
...
- 2023-05-08 - 変異ブロックに関する問題は[#aixcoin-core-dev IRC channel](https://aixcoin-irc.chaincode.com/aixcoin-core-dev/2023-05-08)で最初に報告される
- 2023-05-10 - 修正がマージされる（[#27608](https://github.com/aixcoin/aixcoin/pull/27608)）
