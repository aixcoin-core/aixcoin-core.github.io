---
title: Aixcoin Core 0.19.0.1
id: ja-release-0.19.0.1
name: release-0.19.0.1
permalink: /ja/releases/0.19.0.1/
excerpt: Aixcoin Core バージョン 0.19.0.1 が利用可能になりました
type: releases
layout: release
lang: ja
date: 2018-11-24
...
# Release notes from aixcoin/aixcoin 84908668a0a813ebc61f0c93d63c2a644c677d42
---
...
Aixcoin Core バージョン 0.19.0.1 は以下から入手可能です。
...
datadirを移行する必要がある場合は時間がかかる可能性があります。Aixcoin Coreの
...
macOS Catalinaを実行しているユーザーはAixcoin Core .dmgを開くのに、「右クリック」し
「開く」を選択する必要があります。これはAppleによって課せられた新しい署名要件によるもので、
Aixcoin Coreプロジェクトもまだ遵守していません。
...
- [Reduce memory](https://github.com/aixcoin/aixcoin/blob/master/doc/reduce-memory.md)は、
  メモリが限られているシステム上でAixcoin Coreを実行するための構成の調整を提案しています。(#16339)
...
- `-blockfilterindex`はブロックチェーン全体のBIP158ブロックフィルターの作成を有効にします。
  ...
  注：Aixcoin Coreのこのバージョンは、ローカルユーザーが`getblockfilter` RPCを使って
...
- `whitebind`と`whitelist`は、指定されたインターフェースもしくはIPアドレスを使用して
  ...
  利用可能なパーミッションの詳細については、`aixcoind -help`で出力されるこれら２つのオプションを参照してください。
  (#16248)
...
- 0.18.0では、GUIでBIP70のサポートを無効にするために、`./configure`のフラグが導入されました
  ...
  GUIでBIP70をサポートするAixcoin Coreをコンパイルする場合は、`--enable-bip70`を`./configure`に渡すことができます。 (#15584)
...
- `generate`はAixcoin Core 0.18で廃止された後、削除されました。代わりに`generatetoaddress`
  RPCを使用してください。(#15492)
...
- BIP 61 rejectメッセージはv0.18で廃止されました。このメッセージはデフォルトで
  ...
  BIP 61 rejectメッセージはAixcoin Coreの将来のバージョンで完全に削除されます。(#14054)

- 特に回転式のディスクを持つノードにおける、Aixcoin Coreでよく知られている
  サービス拒否ベクトルを排除するため、`-peerbloomfilters`設定オプションの
  デフォルト値がfalseに変更されました。これによりAixcoin Coreは、
  ...
  旧バージョンのAixcoin CoreおよびBIP37サポートを主導で有効にしたノードに引き続き接続できるはずです。
  ただし、このようなソフトウェアの開発者は、特定のBIP37ノードまたは代替のトランザクション
  フィルタリングシステムを使用して移行すること検討する必要があります。(#16152)

- デフォルトで、Aixcoin Coreはブロックリレー専用に使用される2つの追加のアウトバウンド接続を作成するようになりました。
...
- `aixcoin-cli -getinfo`の`testnet`フィールドの名前が`chain`に変わり、
...
- `getblockchaininfo`は`bip9_softforks`オブジェクトを返さなくなりました。
  ...
  Aixcoin Coreがそのソフトフォークがアクティブかどうか判断する方法（BIP9やBIP90など）を
...
- 複数のピアによって通知されたトランザクションを取得する際、
  以前のバージョンのAixcoin Coreは、ピアのアナウンスを受信した順番でトランザクションが
...
- Tor Hidden Serviceが使用されている場合、別のポートがクリアネット接続用に構成されていても
  Aixcoin Coreは標準ポート8333にバインドされます。これにより同一のデフォルト以外のポート暗号を
...
- v1からv16のwitness version（将来のsegwitバージョン）へ支払うアウトプットを持つ
  ...
  トランザクションが未承認状態のままになる心配をせずに、有効なbech32 Aixcoinアドレスを
  受け入れることができます。 (#15846)
...
- 新しいウォレットフラグ`avoid_reuse`が追加されました（デフォルトはオフ）。
  ...
  これにより、（Aixcoin Core v0.17.0で追加された）「部分的なコイン使用の回避」と共に、
