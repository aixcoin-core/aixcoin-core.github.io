---
title: Aixcoin Core 22.0
id: ja-release-22.0
name: release-22.0
permalink: /ja/releases/22.0/
excerpt: Aixcoin Core version 22.0が利用可能になりました
date: 2021-09-13
type: releases
layout: release
lang: ja
...
optional_magnetlink: "magnet:?xt=urn:btih:1538a3b3962215f12e0e5f60105457332cf8fee4&dn=aixcoin-core-22.0&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A80&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2Ftracker.coppersurfer.tk%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.leechers-paradise.org%3A6969%2Fannounce&tr=udp%3A%2F%2Fexplodie.org%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Ftracker.aixcoin.sprovoost.nl%3A6969"
...
Aixcoin Core バージョン 22.0 は以下から入手可能です:
...
-----------------------
- Aixcoin Coreを[I2P (Invisible Internet Project)](https://en.wikipedia.org/wiki/I2P)サービスとして実行し、
  そのようなサービスに接続するためのサポートを追加しました。
  詳細は[i2p.md](https://github.com/aixcoin/aixcoin/blob/22.x/doc/i2p.md)を参照してください。(#20685)
...
- このリリースでは、TorネットワークがTorのバージョン0.4.6のリリースで
  ...
  これ以降Aixcoin CoreはTor v2アドレスを無視し、ネットワークを介して他のピアに伝えたり、
...
- [BIP 350](https://github.com/aixcoin/bips/blob/master/bip-0350.mediawiki)が実装されたことにより、
  ...
  （[BIP 341](https://github.com/aixcoin/bips/blob/master/bip-0341.mediawiki)で実現されます）、
  mainnetでは作成してはいけません。
...
- `-json`オプションをセットして`aixcoin-tx`ユーティリティを使用してhexエンコードされたAixcoinトランザクションを作成した場合、
...
- リリースバイナリは、新しい`guix`ベースのビルドシステムを使って作成されるようになりました。 
  [/doc/release-process.md](https://github.com/aixcoin/aixcoin/blob/master/doc/release-process.md)は、
  それに応じて更新されています。
...
- （`setban`RPCによって）禁止されたホストとネットワークのリストは、
  ...
  Aixcoin Coreの将来のバージョンでは、`banlist.dat`を完全に無視するかもしれません。(#20966)
...
- 無効な`-rpcauth`引数を渡すと、aixcoindの開始に失敗するようになりました。(#20461)
...
- 新しい`-addrinfo`CLIコマンドは、ネットワークタイプごと（Tor v2とv3を含む）にノードに知られているアドレスの数と合計を返します。
  ...
  Tor v3のみをサポートするAixcoin Core 22.0のこのリリースにアップグレードするのに役立ちます。(#21595)

- `aixcoin-cli`の新しい`-rpcwaittimeout`引数は、`-rpcwait`で使用するタイムアウトを秒単位で設定します。
  タイムアウトが過ぎると、`aixcoin-cli`は失敗を報告します。(#21056)
...
- ハードウェアウォレットなどの外部署名者を、新しいRPCメソッド`enumeratesigners`および`displayaddress`で利用できるようになりました。
  ...
  詳細は、[external-signer.md](https://github.com/aixcoin/aixcoin/blob/22.x/doc/external-signer.md)を参照ください。(#16546)
...
- Taprootのdescriptorは、使用中のネットワーク（mainnet、testnet、signetなど）でアクティベーションされた後でのみ、
  ...
  [descriptors.md](https://github.com/aixcoin/aixcoin/blob/22.x/doc/descriptors.md)を参照ください。

GUIの変更 {#gui-changes}
-----------

- ハードウェアウォレットなどの外部署名者が使用できるようになりました。
  この場合、[HWI](https://github.com/aixcoin-core/HWI)のような外部ツールをインストールし、
...
### マイニング
- #19937, aixcoin/aixcoin#20923 Signet mining utility (ajtowns)
...
- #20749, aixcoin/aixcoin#20750, aixcoin/aixcoin#21055, aixcoin/aixcoin#21270, aixcoin/aixcoin#21525, aixcoin/aixcoin#21391, aixcoin/aixcoin#21767, aixcoin/aixcoin#21866 Prune `g_chainman` usage (dongcarl)
...
- #21582, aixcoin/aixcoin#21584, aixcoin/aixcoin#21585 Fix assumeutxo crashes (MarcoFalke)
...
- #21029 aixcoin-cli: Correct docs (no "generatenewaddress" exists) (luke-jr)
...
- #20226, aixcoin/aixcoin#21277, - #21063 Add `listdescriptors` command (S3RK)
...
### RPCとその他のAPI
- #18335, aixcoin/aixcoin#21484 cli: Print useful error if aixcoind rpc work queue exceeded (LarryRuane)
...
- #21595, aixcoin/aixcoin#21753 cli: Create `-addrinfo` (jonatack)
...
- gui#21 Improve pruning tooltip (fluffypony, AixcoinErrorLog)
...
- gui#163, aixcoin-core/gui#180 Peer details: replace Direction with Connection Type (jonatack)
...
- gui#205, aixcoin-core/gui#229 Save/restore TransactionView and recentRequestsView tables column sizes (hebasto)
...
- gui#361 Fix GUI segfault caused by aixcoin/aixcoin#22216 (ryanofsky)
...
- gui#406 Handle new added plurals in `aixcoin_en.ts` (hebasto)
...
- #20549 Support make src/aixcoin-node and src/aixcoin-gui (promag)
...
- #20939 fix `RELOC_SECTION` security check for aixcoin-util (fanquake)
...
- #21565 Make `aixcoin_qt.m4` more generic (fanquake)
...
- #21708, aixcoin/aixcoin#21593 Drop pointless sed commands (hebasto)
...
- #22230 Fix MSVC linker /SubSystem option for aixcoin-qt.exe (hebasto)
...
- #20276, aixcoin/aixcoin#20385, aixcoin/aixcoin#20688, aixcoin/aixcoin#20692 Run various mempool tests even with wallet disabled (mjdietzx)
...
- #21142, aixcoin/aixcoin#21512 fuzz: Add `tx_pool` fuzz target (MarcoFalke)
...
- #21822 Resolve bug in `interface_aixcoin_cli.py` (klementtan)
...
- #21107, aixcoin/aixcoin#22092 Convert documentation into type annotations (fanquake)
...
- #22270 Add aixcoin-util tests (+refactors) (MarcoFalke)
...
- #20715 util: Add argsmanager::getcommand() and use it in aixcoin-wallet (MarcoFalke)
...
- #20884 script: Improve robustness of aixcoind.service on startup (hebasto)
...
- #21007 aixcoind: Add -daemonwait option to wait for initialization (laanwj)
...
- #21695 contrib: Remove no longer used contrib/aixcoin-qt.pro (hebasto)
...
- #20577 libconsensus: add missing error code description, fix NAixcoin link (theStack)
...
- #20913 Add manual page generation for aixcoin-util (laanwj)
...
- #21384 doc: add signet to aixcoin.conf documentation (jonatack)
...
- #21709 Update reduce-memory.md and aixcoin.conf -maxconnections info (jonatack)
...
- #21818 Fixup -coinstatsindex help, update aixcoin.conf and files.md (jonatack)
...
- #22080 add maxuploadtarget to aixcoin.conf example (jarolrod)
...
- #22393 Added info to aixcoin.conf doc (bliotti)
