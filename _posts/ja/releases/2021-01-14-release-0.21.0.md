---
title: Aixcoin Core 0.21.0
id: ja-release-0.21.0
name: release-0.21.0
permalink: /ja/releases/0.21.0/
excerpt: Aixcoin Core バージョン 0.21.0 が利用可能になりました
date: 2021-01-14
type: releases
layout: release
lang: ja
...
Aixcoin Core バージョン 0.21.0 は以下から入手可能です。
...
- このリリースでは、Tor v3 hidden serviceのサポートが追加され、
  [BIP155](https://github.com/aixcoin/bips/blob/master/bip-0155.mediawiki)を使用して
  ネットワーク経由で他にピアにそれらが拡散されます。v2 hidden serviceは引き続き
  Aixcoin Coreでサポートされていますが、Torネットワークは今後数ヶ月のうちに
...
- `-listenonion`設定パラメーターを設定することで自動的に作成されるTor onion serviceは、
  ...
  Aixcoin Coreがそれを読み取ることはありません。Tor v3 serviceの秘密鍵は、
...
- このリリースでは、`-blockfilterindex=1 -peerblockfilters=1`で有効にすると、
  ネットワーク上のピアに[BIP157](https://github.com/aixcoin/bips/blob/master/bip-0157.mediawiki)
  Compact Filterを提供するためのサポートが追加されています。(#16442)

- このリリースでは、既存のmainnet、testnetおよびregtestネットワークに加えて、
  signet([BIP325](https://github.com/aixcoin/bips/blob/master/bip-0325.mediawiki))の
...
- このリリースでは[BIP339](https://github.com/aixcoin/bips/blob/master/bip-0339.mediawiki)
  wtxidリレーが実装されています。ネゴシエートされると、トランザクションはtxidではなく
  wtxidを使って通知されます。(#18044)

- このリリースでは、Taprootコンセンサスルール([BIP341](https://github.com/aixcoin/bips/blob/master/bip-0341.mediawiki)
  および[BIP342](https://github.com/aixcoin/bips/blob/master/bip-0342.mediawiki))を実装しています。
...
- `startupnotify`オプションは、Aixcoin Coreが起動シーケンスを終えた時に
...
Aixcoin Coreは起動時に新しいウォレットを自動的に作成しなくなりました。
コマンドラインもしくは`aixcoin.conf`か`settings.json`ファイルの`-wallet`オプションで
指定された既存のウォレットをロードします。また、デフォルトではトップレベルの名前のない("")
ウォレットもロードされます。ただし、指定されたウォレットが存在しない場合、
Aixcoin Coreはこれまでのリリースのように新しい鍵とアドレスで新しいウォレットを作成するのではなく、
警告をログに記録するようになりました。

新しいウォレットは（より目立つウォレット作成オプションのある）GUIや、`aixcoin-cli createwallet`および
`aixcoin-wallet create`コマンド、`createwallet` RPCを介して作成できます。(#15454, #20186)
...
バグや不足している機能は[Issue Tracker](https://github.com/aixcoin/aixcoin/issues)に報告できます。
...
既にAixcoin Coreウォレットを持っているユーザーは、動作を変更することなく、これまでと同じように
引き続き使用できます。新しく作成された（デフォルトタイプのウォレットのままの）レガシーウォレットは、
これまでのバージョンのAixcoin Coreと同じように動作します。
...
descriptorを使用するように変更したことで、Aixcoin Coreが使用するデフォルトの導出パスが
...
- vbyte毎のsatoshi(sat/vB)で表記される新しい`fee_rate`パラメータ/オプションが`sendtoaddress`および`sendmany`、
  ...
  `fundrawtransaction`と`walletcreatefundedpsbt`のレガシーな`feeRate`オプションは、1,000 vbytes毎のAIX(AIX/kvB)の
  手数料率を設定するためにまだ存在していますが、混乱を避けるため間もなく廃止されると予想されています。
  これらのRPCについては、手数料率のエラーメッセージがAIX/kBからsat/vBに更新され、ヘルプドキュメント内の
  AIX/kBがAIX/kvBに更新されました。`send`および`sendtoaddress`の例が更新され、明示的な手数料率を使用した
  トランザクションを作成する際に役立つようになりました。(#20305, #11413)

- `bumpfee` RPCの`fee_rate`オプションがAIX/kvBからsat/vBに変更され、ヘルプドキュメントが更新されました。
  これは重大なAPIの変更であるとユーザーに警告されますが、比較的害はないはずです。
  AIX/kvBとsat/vBの単位の間には大きな差（100,000倍）があるため、手数料率がsat/vBではなくAIX/kvBで誤って計算された
...
- GUIで作成またはロードされたウォレットは起動時に自動的にロードされるようになったため、
  次回Aixcoin Coreを起動した際に手動で再ロードする必要はありません。
  起動時にロードするウォレットのリストは、`\<datadir\>/settings.json`に保存され、
  コマンドライン引数や`aixcoin.conf`の`-wallet=`設定でロードするウォレットを
