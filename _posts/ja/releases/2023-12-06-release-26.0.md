---
title: Aixcoin Core 26.0
id: ja-release-26.0
name: release-26.0
permalink: /ja/releases/26.0/
excerpt: Aixcoin Core バージョン 26.0 が利用可能になりました
date: 2023-12-06
type: releases
layout: release
lang: ja
...
optional_magnetlink: "magnet:?xt=urn:btih:12f43c4ac1f4cddc6095c17ffb784ccf93446081&dn=aixcoin-core-26.0&tr=udp%3A%2F%2Ftracker.openbittorrent.com%3A80&tr=udp%3A%2F%2Ftracker.opentrackr.org%3A1337%2Fannounce&tr=udp%3A%2F%2Ftracker.coppersurfer.tk%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.leechers-paradise.org%3A6969%2Fannounce&tr=udp%3A%2F%2Fexplodie.org%3A6969%2Fannounce&tr=udp%3A%2F%2Ftracker.torrent.eu.org%3A451%2Fannounce&tr=udp%3A%2F%2Ftracker.aixcoin.sprovoost.nl%3A6969&ws=http%3A%2F%2Faixcoin-core.github.io%2Fbin%2F"
...
Aixcoin Core バージョン 26.0 は以下から入手可能です:
...
Aixcoin CoreはLinuxカーネルやmacOS 11.0以降、Windows 7以降を使用している
...
- [BIP324](https://github.com/aixcoin/bips/blob/master/bip-0324.mediawiki)で定義された
...
- [Miniscript](https://aixcoin.sipa.be/miniscript/)式が、ディスクリプターを使用して動作するすべてのRPCに対して、
  Taprootディスクリプターで使用できるようになりました。(#27255)

- `finalizepsbt`は、[Miniscript](https://aixcoin.sipa.be/miniscript/)互換のTaprootリーフを使用するインプットでPSBTをファイナライズできるようになりました。(#27255)
...
- `loadtxoutset`が追加され、`dumptxoutset`で生成されたフォーマットのUTXOスナップションをロードできるようになりました。
  ...
  その結果、数時間ではなく数分で、ネットワークの先端と同期した状態のaixcoindインスタンスが出来上がります。
  UTXOスナップショットは、通常サードパーティのソース（HTTP、torrentなど）を介して取得されますが、
  その内容はハッシュによってチェックされるためこれは合理的です。

  このプロセスの詳細については、`assumeutxo`の設計ドキュメント(<https://github.com/aixcoin/aixcoin/blob/master/doc/design/assumeutxo.md>)を参照ください。
...
- `aixcoind`および`aixcoin-qt`は、使用されるdatadirに無視されるaixcoin.confファイルが含まれている場合、
  起動時にエラーを発生させるようになりました。これは、aixcoin.confファイルにdatadir=の行がある場合に発生する可能性があります。
  このエラーメッセージは、偶発的な設定ミスを防ぐための単なる診断で、無効にすることで、
  datadirに含まれるaixcoin.confを無視してdatadirを使用するという以前の挙動に戻すことができます。(#27302)
...
- 新しい`aixcoinconsensus_verify_script_with_spent_outputs`関数がlibconsensusで利用可能になり、
  検証中のトランザクションの使用済みアウトプットをオプションで受け入れます。
- 新しい`aixcoinconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT`フラグがlibconsensusで利用可能になり、
...
- このリリースでは、ウォレットのロード処理が変更されました。これまでは（警告付きで）ロードすることができた、
  ...
  この問題が発生した場合は、以前のバージョンのAixcoin Coreにウォレットをロードし、
...
- ディスクリプターウォレット用のTaprootリーフ内で[Miniscript](https://aixcoin.sipa.be/miniscript/)が使用できるようになりました。(#27255)
