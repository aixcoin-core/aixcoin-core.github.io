---
title: Aixcoin Core 0.20.0
id: ja-release-0.20.0
name: release-0.20.0
permalink: /ja/releases/0.20.0/
excerpt: Aixcoin Core バージョン 0.20.0が利用可能になりました
date: 2020-06-03
type: releases
layout: release
lang: ja
...
# Release notes from aixcoin/aixcoin a62f0ed64f8bbbdfe6467ac5ce92ef5b5222d1bd
...
0.20.0リリースノート {#release-notes}
====================

Aixcoin Core バージョン 0.20.0 は以下から入手可能です。
...
データディレクトリを移行する必要がある場合は時間がかかる可能性があります。Aixcoin Coreの
旧バージョンのウォレットが一般的にサポートされます。

互換性 {#compatibility}
==============

Aixcoin CoreはLinuxカーネルやmacOS 10.12以降、Windows 7以降を使用している
オペレーティングシステムでサポートされ、広範囲にテストされています。
Aixcoin Coreは他のほとんどのUNIXライクなシステムでも動作するはずですが、
それらの環境では頻繁にテストされていません。
サポートされていないシステムでAixcoin Coreを使用するのはお勧めできません。

Aixcoin Core 0.20.0以降、10.12より前のmacOSはサポートされなくなりました。
さらに、macOSのダークモードが有効になっていてもAixcoin Coreの外観は変わりません。
...
- 単純に`make`を実行するのではなく、`AIXCOIN_GENBUILD_NO_GIT=1 make`を実行してください。
...
#### Aixcoin CoreからBIP61 rejectメッセージが削除される

BIP61を有効にする`-enablebip61`コマンドラインオプションが削除されました。(#17004)

この機能はAixcoin Core 0.18.0からデフォルトでは無効になっています。
ネットワーク上のノードは、一般的に有効なメッセージ（rejectメッセージを含む）を
送信するものと信頼できないため、この機能は信頼できるノードに接続されている場合のみ
使用する必要がありました。この削除された機能に依存している場合は、以下に推奨する代替手段を
使用してください。

- テストやAixcoinのP2Pネットワークプロトロルの実装のデバッグでは、Aixcoin Coreの
  最近のバージョンで導入されたログメッセージを検査してください。Aixcoin Coreは
  デバッグメッセージ(`-debug=<category>`)をストリーム(`-printtoconsole`)
  もしくはファイル(`-debuglogfile=<debug.log>`)に記録します。

- ブロックの有効性をテストする際は、以下のRPCを使用します：

  - `submitblock`

  - 無効なPOWを持つブロックについては、`'mode'`を`'proposal'`にセットした`getblocktemplate`

- トランザクションの有効性をテストする際は、以下のRPCを使用します：

  - `sendrawtransaction`

  - `testmempoolaccept`

- ウォレットはrejectメッセージがないからといってトランザクションがネットワークに伝搬されたと
  想定しないでください。その代わり、ネットワーク上の他のピアからトランザクションが通知されるのを
  検知してください。ウォレットはrejectメッセージがないからといって、トランザクションが適切な
  手数料を支払っていると思い込んではいけません。代わりに手数料の見積もりを使って手数料を設定し、
  希望する時間内にトランザクションが承認されなかった場合はreplace-by-feeを使って手数料を増やすべきです。

BIP61のrejectメッセージサポートの削除は、以下のようなマイナーなRPCとロギングの変更も含みます：

- `testmempoolaccept`と`sendrawtransaction`はトランザクションがmempoolに受け入れられなくても
  P2P rejectコードを返さなくなりました。ただし、まだreject理由は返します。

- トランザクションがmempoolに受け入れられなかった場合、以前はrejectコードを報告していた
  ログメッセージは、rejectコードを報告しなくなりました。rejectの理由はまだ報告されます。

更新されたRPC {#updated-rpcs}
------------

- descriptorを受け入れるRPCをは新しく`sortedmulti(...)` descriptor タイプを
  受け入れるようになりました。このdescriptorは結果のスクリプトで公開鍵が辞書順に
  ソートされるマルチシグスクリプトをサポートします。(#17056)

- `walletprocesspsbt`RPCと`walletcreatefundedpsbt`RPCは、
  公開鍵のBIP32導出パスが分かる場合、デフォルトでその導出パスを含むようになりました。
  これは`bip32derivs`パラメータに`false`をセットすることで無効にすることができます。(#17264)

- `bumpfee`RPCの`totalFee`パラメータは0.19で非推奨になっていましたが、削除されました。(#18312)

- `bumpfee`RPCは、秘密鍵が無効になっているウォレットで使用すると、
  PSBTを返します。(#16373)

- `getpeerinfo`RPCは、ピア選択の多様化のために使用されるマップされたAutonomous System
  を示す`mapped_as`フィールドを含むようになりました。以下の _新しい設定_ に記載している
  `-asmap`設定オプションを参照してください。(#16702)

- `createmultisig`RPCと`addmultisigaddress`RPCは、新しく作成されたアドレスの
  output script descriptorを返すようになりました。(#18032)

ビルドシステム {#build-system}
------------

- OpenSSLがAixcoin Coreでは使用されなくなりました。(#17265)

- Aixcoin CoreからBIP70のサポートが完全に削除されました。
  `--enable-bip70`オプションは残りますが、設定するとエラーが投げられます。(#17165)

- リリースバイナリを実行するにはglibc 2.17以上が必要になりました。
  これによりRHEL 7, CentOS 7, Debian 8およびUbuntu 14.04 LTSとの互換性が維持されます。(#17538)

- gitianビルドで提供されるソースコードアーカイブには、autotoolsの成果物は一切含まれていません。
  したがって、そのようなソースからビルドする際は、解凍したアーカイブのルートから`./autogen.sh`
  スクリプトを実行しなければなりません。これは、`autotools`やその他の必要なパッケージが
  ユーザーのシステムにインストールされていることを意味します。(#18331)

新しい設定 {#new-settings}
------------

- 新しい設定パラメータ`rpcwhitelist`と`rpcwhitelistdefault`が追加され、
  特定のRPCとユーザー対し、一部のRPC呼び出しのみを許可することができようになりました。(#12763)
  
- 新しく`-asmap`設定オプションが追加され、IPアドレスをAutonomous System Numbers (ASN)
  にマッピングし、単一のASNへの接続数を制限することで、ノードのネットワーク接続を多様化することが
  できるようになりました。詳細については課題 #16599 およびPR #16702 および`aixcoind help`
  を参照してください。このオプションは実験的なもので、将来のリリースで削除されたり変更されたりする
  可能性があるため、従来のIPアドレスの/16プレフィックスマッピングはデフォルトのままです。(#16702)

更新された設定 {#updated-settings}
----------------

- Aixcoin Coreの起動時に、設定されたすべてのカスタム設定が`debug.log`ファイルに
  書き込まれるようになり、トラブルシューティングを支援するようになりました。(#16115)

- `bootstrap.dat`ファイルを介した起動時のブロックのインポートはデフォルトでは
  行われなくなりました。ファイルは`-loadblock=<file>`で指定しなければならなくなりました。(#17044)

- ロギングカテゴリ`-debug=db`は、`coindb`と区別するため`-debug=walletdb`にリネームされました。
  `-debug=db`オプションは非推奨となり、次のメジャーリリースで削除される予定です。(#17410)

- 設定パラメータ`-walletnotify`は、引数の`%w`を通知を生成するウォレットの名前に置き換えます。
  これはWindowsではサポートされていません。(#13339)

削除された設定 {#removed-settings}
----------------

- 設定パラメータ`-whitelistforcerelay`は、バージョン0.13で無効になっており、
  実際にはほぼ4年間サポートされていないことが判明したため、削除されました。(#17985)

GUIの変更 {#gui-changes}
-----------

- 「システムログイン時にAixcoin Coreを起動」オプションがmacOSで削除されました。(#17567)

- Peersウィンドウでピアの詳細に、ピア選択の多様化のために使用されるマップされた
  Autonomous Systemを示す`Mapped AS`が表示されるようになりました。
  上記の _新しい設定_ の`-asmap`設定オプションを参照してください。(#18402)

- バージョン0.18のリリースノートで[発表された](https://aixcoin-core.github.io/aixcoincore.org/ja/releases/0.18.0/#wallet-gui)
  「既知の問題」が修正されました。この問題は複数のAixcoin CoreウォレットとGUIのコインコントロール機能を
  同時に使用している人に影響がありました。(#18894)

- watch-onlyウォレットについて、送金画面で新しいトランザクションを作成したり、
  トランザクション画面で既存のトランザクションの手数料をバンプすると、
  Partially-Signed Aixcoin Transaction (PSBT)が自動的にシステムクリップボードに
  コピーされます。そしてこれを[HWI](https://github.com/aixcoin-core/HWI)などの
  外部プログラムに貼り付けて署名することができます。Aixcoin Coreの将来のバージョンでは、
  PSBTをファイナライズしブロードキャストするためのGUIオプションをサポートするはずですが、
  現時点では`finalizepsbt`RPCおよび`sendrawtransaction`RPCをデバッグコンソールで使用できます。(#16944, #17492)

ウォレット {#wallet}
------

- ウォレットはRPCを使用時にデフォルトでbech32アドレスを使用し、ネイティブsegwitの
  お釣り用アウトプットを作成するようになりました。(#16884)

- アウトプットトラストの計算方法が修正され、承認/未承認の残高状態やコイン選択に
  影響があります。(#16766)

- `gettransaction`RPCおよび`listtransactions`RPC、`listsinceblock`RPCのレスポンスに、
  ウォレットトランザクションを含むブロックの高さがあれば、そのブロックの高さも表示されるようになりました。(#17437)

- `getaddressinfo`RPCで`label`フィールドが非推奨になりました（このリリースでは`-deprecatedrpc=label`
  設定パラメータを使って再有効化することができます）。`labels`フィールドはJSONオブジェクトを返すものから、
  ラベル名のJSON配列を返すものに変更されました（このリリースでは`-deprecatedrpc=labelspurpose`
  設定オプションを使って以前の動作を再有効化することができます）。非推奨の設定パラメータを使った下位互換性は、
  0.21のリリースでは削除される予定です。(#17585, #17578)

ドキュメントの変更 {#documentation-changes}
---------------------

- Aixcoin Coreの自動生成されたソースコードドキュメントは
  https://doxygen.aixcoincore.org で公開されています。(#17596)
...
- `-getinfo`パラメータを使って実行される`aixcoin-cli`ユーティリティは、
  ベストヘッダーチェーン上でダウンロードされたブロックヘッダーの数を持つ
  `headers`フィールド（同様に`-getinfo`で返される`blocks`と似ています）と、
  ローカルノード上でベストブロックチェーンがどれだけ同期されたかを推定する
  `verificationprogress`フィールドを返すようになりました。
  返される情報には`protocolversion`および`walletversion`、`keypoololdest`
  フィールドが含まれなくなりました。(#17302, #17650)

- `aixcoin-cli`ユーティリティは、`-stdinwalletpassphrase`パラメータを受け入れるようになりました。
  これは`walletpassphrase`RPCおよび`walletpassphrasechange`RPCを呼び出す際に使用することができ、
  ターミナルにエコーすることなく標準入力からパスフレーズを読み取ることができるようになり、
  あなたの画面を見ている人に対するセキュリティが向上します。既存の`-stdinrpcpass`パラメータも更新され、
  パスフレーズをエコーしません。 (#13716)

コマンドライン {#command-line}
------------

- `-main.port=8333` `-test.server=1`のようなmain/test/regtestのネットワーク名を
  先頭にしたコマンドラインオプションは、以前は許可されているものの無視されていました。
  今回これらのオプションは起動時に"Invalid parameter"を発生させるようになりました。(#17482)

新しいRPC {#new-rpcs}
--------

- `dumptxoutset`RPCは現在のUTXOセットのシリアライズされたスナップショットを出力します。
  特定のブロック高のUTXOセットのスナップショットを生成するためのスクリプトが
  `contrib/devtools`ディレクトリに用意されています。(#16899)

- `generatetodescriptor`RPCはregtestモードを使うテスターが、
  任意のoutput script descriptorに支払うブロックを生成できます。(#16943)

更新されたRPC
------------

- `verifychain`RPCのデフォルト値は、コマンドラインオプションや設定ファイル
  (`-checklevel`および`-checkblocks`)に依存するのではなく、静的なものになりました。
  デフォルト値に依存したくない場合、ユーザーは明示的にRPC引数を渡すことができます。(#18541)

- `getblockchaininfo`RPCの`verificationprogress`フィールドは、1より高い値を
  報告しなくなりました。以前はチェーンが100%以上検証されていたことを報告することがありました。(#17328)

テスト {#tests}
-----

- testnetやregtestネットワークで実行している場合、設定ファイルで不適切な`walletdir=path`
  設定がされているとエラーになります。設定は`chain.walletdir=path`とするか、
  適切な`[chain]`セクションに配置する必要があります。(#17447)

- `-fallbackfee`はmainチェーンではデフォルトで0（無効）でしたが、testチェーンではデフォルトで
  0.0002でした。現在はすべてのチェーンでデフォルト0になっています。testnetとregtestのユーザーは、
  `-fallbackfee`オプションを設定しておらず、以前と同じように動作させたい場合は、
  設定`fallbackfee=0.0002`を追加する必要があります。(#16524)

ビルドシステム
------------

-  Android Native Development Kit (NDK)でビルドするためのサポートが提供されました。(#16110)

0.20.0のチェンジログ {#change-log}
=================
...
[Transifex](https://www.transifex.com/aixcoin/aixcoin/)での翻訳を手伝ってくれたみなさんもありがとうございます。
{% endgithubify %}
