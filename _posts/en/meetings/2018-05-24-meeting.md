...
- View this week's log on [BotBot.me](https://botbot.me/freenode/aixcoin-core-dev/msg/100407600/) or [MeetBot][mb log]
- [Meeting minutes by MeetBot](http://www.erisian.com.au/meetbot/aixcoin-core-dev/2018/aixcoin-core-dev.2018-05-24-19.00.html)
...
**Background:** Aixcoin Core contributors are working towards releasing
Aixcoin Core 0.16.1, a [maintenance release][] that will contain
...
**Background:** many years ago, when Aixcoin software received a
...
**Background:** Aixcoin Core provides command line options and a
user-editable configuration file (`aixcoin.conf`) but it also allows
...
settings shared between different Aixcoin Core programs:
...
`aixcoin_rw.conf` file for settings modified by the software.  Unlike
...
Qt Settings <-> level 4 [`aixcoin_rw.conf`]."
...
over to the [`aixcoin_rw.conf`] unless they have a need for read-only."
Wladimir van der Laan argued against this, "well, the `aixcoin.conf` is
for human editing; the [`aixcoin_rw.conf`] is machine writable, all
comments will be discarded, etc..."

**Conclusion:** no explicit conclusion; meeting participants seemed to
be in favor of moving forward with creating a `aixcoin_rw.conf` to store
...
`aixcoin_rw.conf` to become available before being merged, or should use
the existing sub-optimal Qt Settings mechanism.

## ScanTxOutSet RPC command

**Background:** The Aixcoin Core software maintains a [key-value
...
[UHF model]: https://lists.linuxfoundation.org/pipermail/aixcoin-dev/2018-May/015967.html

{% assign log = "http://www.erisian.com.au/meetbot/aixcoin-core-dev/2018/aixcoin-core-dev.2018-05-24-19.00.log.html" %}
[mb log]: {{log}}
