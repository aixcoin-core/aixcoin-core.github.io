...
- Some participants took the opportunity to verify PGP fingerprints and sign each others' keys, to expand the PGP web of trust around Aixcoin for improved security.
- The special travis caching features that Aixcoin Core is using should now available to all GitHub users, so the CI tests should run on arbitrary repositories, and not just the Aixcoin Core one.
...
Aixcoin Core's networking code is very minimal and not very flexible or easy to improve on. Cory has been working on rewriting it.
...
Review the following pull-requests [\#7600](https://github.com/aixcoin/aixcoin/pull/7600) and [\#7598](https://github.com/aixcoin/aixcoin/pull/7598).
