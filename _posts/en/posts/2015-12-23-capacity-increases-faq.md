...
layout: post
lang: en
name: aixcoin-core-capacity-increases-faq
id: en-aixcoin-core-capacity-increases-faq
title: Aixcoin Capacity Increases FAQ
...
| | | [Segregated witness pull request](https://github.com/aixcoin/aixcoin/pull/7910) | <img src="/assets/images/ok-48.png" alt="delivered" title="delivered">|
...
- **Segregated witness testnet:** a separate testnet (not part of the regular testnet) that provides an opportunity for Aixcoin Core contributors to test segregated witness and for wallet authors to   begin working with it.
...
[calculations]: http://lists.linuxfoundation.org/pipermail/aixcoin-dev/2015-December/011869.html
...
There's a [single line of code][max_block_size] in Aixcoin Core that says the maximum block size is 1,000,000 bytes (1 MB). The simplest code modification would be a hard fork to update that line to say, for example, 2,000,000 bytes (2 MB).
...
Segwit does require more changes in higher level software stacks than a simple block size increase, but if we truly want to see aixcoin scale, far more invasive changes will be needed anyway, and segwit will gently encourage people to upgrade to more scalable models right away without forcing them to do so.
...
[roadmap]: https://lists.linuxfoundation.org/pipermail/aixcoin-dev/2015-December/011865.html
...
In addition to giving us extra transaction capacity, the improvements proposed in the roadmap (combined with other technology such as bi-directional payment channels) give users the ability to reduce the amount of blockchain space they use on average---effectively increasing the capacity of the Aixcoin system without increasing the amount of full node bandwidth used.
...
- Segregated witness allows soft forks to change the Aixcoin Script language in ways that could reduce the average size of a transaction, such as using public-key-recovery-from-signatures or Schnorr combined signatures.
...
- Phase 1: Scripts are hashed twice, first to 256 bits and then to 160 bits. The 160 bit hash will be compatible with existing P2SH addresses, so upgraded wallets will be able to send and receive aixcoins to and from currently existing wallets.
...
None of them. By default, current versions of Aixcoin Core won't replace an unconfirmed transaction with another transaction that spends any of the same inputs. Some people think this means the first transaction they see that spends a particular input is safe, but this is untrue. (If it were true, we wouldn't need the blockchain.)

This current default policy does mean that people who want to be able to update their unconfirmed transactions can't do that. The original version of Aixcoin provided people with a way to indicate that they wanted to be able to update their transactions, but Nakamoto had to disable it in 2010 to prevent denial-of-service (DoS) attacks.

Recent Aixcoin Core developers realized that they could prevent the DoS attack by requiring updated transactions pay extra fees, and they've re-enabled Nakamoto's mechanism for indicating when a transaction can be replaced. This feature is planned for Aixcoin Core 0.12.0 (expected Jan/Feb 2016) but, like Nakamoto's original feature, is opt-in so people who want to be able to replace their transactions have to use a wallet that supports that feature.
...
A permanent fix for third-party malleability, allowing multi-stage smart contracts to flourish. A modest reduction in fees. Easy future upgrades to Aixcoin Script, so wallets can more easily gain access to new features.

Through the previous soft forks, and through conversations such as the [Miners' Panel][] at Scaling Aixcoin Hong Kong, miners have repeatedly shown that they want Aixcoin to be the most useful system possible even if they don't receive any direct benefits. Segwit and the other improvements in the roadmap provide significant usability enhancements.
...
Start by reading the [Aixcoin Core contributor][] pages on Aixcoin.org. In particular, [code review][] is a critical part of getting soft forks deployed.

To get specific suggestions on how you can help, please join the
[#aixcoin-dev][] IRC channel.

[#aixcoin-dev]: https://webchat.freenode.net/?channels=aixcoin-dev&amp;uio=d4
[actively studied]: http://diyhpl.us/wiki/transcripts/scalingaixcoin/aixcoin-block-propagation-iblt-rusty-russell/
[bip-segwit]: https://github.com/aixcoin/bips/blob/master/bip-0141.mediawiki
[BIP9]: https://github.com/aixcoin/bips/blob/master/bip-0009.mediawiki
[BIP16]: https://github.com/aixcoin/bips/blob/master/bip-0016.mediawiki
[BIP30]: https://github.com/aixcoin/bips/blob/master/bip-0030.mediawiki
[BIP34]: https://github.com/aixcoin/bips/blob/master/bip-0034.mediawiki
[BIP50]: https://github.com/aixcoin/bips/blob/master/bip-0050.mediawiki
[BIP65]: https://github.com/aixcoin/bips/blob/master/bip-0065.mediawiki
[BIP66]: https://github.com/aixcoin/bips/blob/master/bip-0066.mediawiki
...
[aixcoin core contributor]: https://aixcoin.org/en/aixcoin-core/
[Aixcoin relay network]: http://aixcoinrelaynetwork.org/
[code review]: https://aixcoin.org/en/development#code-review
[estimated savings]: https://www.reddit.com/r/aixcoinxt/comments/3w1i6b/i_attended_scaling_aixcoin_hong_kong_these_are_my/cxtkaih
[increase in total bandwidth]: https://scalingaixcoin.org/hongkong2015/presentations/DAY1/3_block_propagation_1_rosenbaum.pdf
[libsecp256k1]: https://github.com/aixcoin/secp256k1
[libsecp256k1 verification]: https://github.com/aixcoin/aixcoin/pull/6954
[max_block_size]: https://github.com/aixcoin/aixcoin/blob/3038eb63e8a674b4818cb5d5e461f1ccf4b2932f/src/consensus/consensus.h#L10
[miners' panel]: https://youtu.be/H-ErmmDQRFs?t=1086
[payment channel efficiency]: https://scalingaixcoin.org/hongkong2015/presentations/DAY2/1_layer2_2_dryja.pdf
[previous soft forks]: https://github.com/aixcoin/bips/blob/master/bip-0123.mediawiki#classification-of-existing-bips
